...
import org.lopez.springboot.mybatis.mapper.*;
import org.lopez.springboot.mybatis.model.*;
...
public class SqlDataGeneratorTest {
    ...
    private String convertDate(String dateString) {
        if (StringUtils.isEmpty(dateString) || "0".equals(dateString)) {
            return "'0'";
        }
        DateTimeFormatter formatterMysql = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        DateTimeFormatter formatterOracle = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm:ss");
        LocalDateTime localDateTime = LocalDateTime.parse(dateString, formatterMysql);
        String oracleFormat = localDateTime.format(formatterOracle);
        return "to_date('" + oracleFormat + "', 'yyyy-MM-dd hh24:mi:ss')";
    }
